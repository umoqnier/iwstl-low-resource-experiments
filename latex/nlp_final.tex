...
\author[Barriga]{
    ...
    \texttt{dbarriga@ciencias.unam.mx}, \texttt{amilkar.edgar@gmail.com} \\
}
\institute[UNAM]{
...
\begin{frame}
    \begin{figure}
        ...
        \caption{Ejemplo de sistema de traducción automática del habla. Tomada de \url{https://research.google/blog/real-time-speech-to-speech-translation/}}
    \end{figure}
\end{frame}
...
\begin{frame}{Modelos Masivos}
    \begin{itemize}
        ...
        \item <4-> Para adaptar estos modelos a tareas específicas, se utiliza el ajuste fino (\textit{fine-tuning}), donde se entrena el modelo con un conjunto de datos específico para la tarea.
    \end{itemize}
\end{frame}
...
\begin{frame}{Diversidad lingüística en América Latina}
    ...
    \begin{itemize}
        ...
        \item <3-> Todas las lenguas originarias de México son de bajos recursos digitales. Una realidad para muchas lenguas originarias en América Latina.
        \item <4-> Hay muy poca tecnología del lenguaje desarrollada para estas lenguas.
        \item <5-> La mayoría de las lenguas originarias no tienen sistema de escritura estandarizado. Son lenguas principalmente orales.
    \end{itemize}
...
\begin{frame}{Ajuste fino (\textit{Fine-Tuning})}
    ...
    % TODO: Imagen de fine-tuning
\end{frame}

\begin{frame}{Ajuste fino eficiente}
   En el contexto de los bajos recursos, un \textit{Full Fine\-Tuning} causaría \textbf{overfitting} y es computacionalmente costoso. Nuestra Solución: \pause

    \huge \textbf{Parameter-Efficient Fine-Tuning (PEFT)} \cite{xu2023parameterefficientfinetuningmethodspretrained} 
\end{frame}

\begin{frame}{PEFT}

% TODO abundar en la estraegía Adapters
...
\begin{itemize}
    ...
    \item <3-> \textbf{Estrategia específica:} Utilizamos la estrategia específica de \textbf{Adapters} \cite{houlsby2019parameterefficienttransferlearningnlp} para ajustar el \textit{Encoder} que capturar la acústica de las nuevas lenguas.
\end{itemize}
...
\begin{frame}{Modelos pre-entrenados II: SeamlessM4T}
    ...
    \begin{figure}
    \includegraphics[width=0.3\linewidth]{img/meta_PNG12-1353596828.png}
    \end{figure}
...
\begin{frame}{Arquitectura}
    \begin{figure}
        ...
        \caption{Arquitectura de Canary con Adapters lineales.}
    \end{figure}
\end{frame}
...
\begin{frame}
    TODO: Reproducir un audio y
    \begin{exampleblock}{Ejemplos}
        % TODO: Más ejemplos?
        \begin{itemize}
            \item \textit{Kimkevi chi l'awen' pewmamu pu.}
...
\begin{frame}{Aumentación de datos}
    ...
    TODO: Hablar de la estrategia de aumentación de datos que implementamos, y mostrar ejemplos de los audios aumentados. 
...
\begin{frame}{Curvas}
    ...
    TODO: Mostrar curvas de entrenamiento
...
\begin{frame}{Métricas}
    \begin{table}
        \begin{center}
            ...
            Main &  $2.5$       & $1.015$ \\
            Vanilla & $2.3$     & $1.05$  \\
            Filtered\footnote{Filtramos los audios de más de 15 segundos. Entrenamos con el resto} & \textbf{$1.56$}  & \textbf{$3.7$}\\\hline
            \end{tabular}
            % TODO: En el caption poner que es MAIN, Vanilla y Filtered
            \caption{Estas métricas reportadas para nuestros experimentos.}
            \label{tbl:results}
        \end{center}
    \end{table}
\end{frame}
...
\section{Concluisones y trabajo futuro}
...
Edgar Gazque - \url{amilkar.edgar@gmail.com}
\end{frame}
\end{document}
